  ...
  <v-layout>
    <v-row ref="table" fill-height>
      ...
      >
        ...
        <v-btn fab absolute dark color="blue" class="card-add-button" @click="newTask(column.id)">
      ...
      <v-card>
        <v-card-text>
          ...
          <div v-else class="body-1 pt-6">
            The is currently edited by someone else.
      ...
      <v-card>
        <v-card-text>
          <div class="body-1 pt-6">
            Someone has requested the permission to edit this you are currently working on.
...
export default {
  ...
  methods: {
    ...
    editTask (task) {
      this.taskInstance = {..._.pick(task, 'id', 'text'), updatedAt: task.updated_at}
      this.editTaskOpened = true
    },
    newTask (col) {
      this.taskInstance = {text: '', col}
      this.editTaskOpened = true
    },
    saveTask () {
      ...
      axios.request({
        ...
        this.$snack({type: 'success', text: this.taskInstance.id ? 'Updated' : 'New Item Created'})
